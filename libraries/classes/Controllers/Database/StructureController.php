...
class StructureController extends AbstractController
{
    ...
    public function __invoke(): void
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['action']) && $_POST['action'] === 'generate_ai_sql') {
        ...
        if (empty($this->dbIsSystemSchema)) {
            ...
            $createTable = $this->template->render('database/create_table', [
                'db' => $this->db,
                'ai_generated_sql' => $aiGeneratedSql,
                'ai_error' => $aiError,
                'ai_success' => $aiSuccess,
                'ai_table_preview' => $aiTablePreview,
                'user_prompt' => $_POST['ai_description'] ?? '',
            ]);
        }
    ...
    {
        $userPrompt = isset($_POST['ai_description']) ? trim($_POST['ai_description']) : '';
        $userTableName = isset($_POST['table_name']) ? trim($_POST['table_name']) : '';
        ...
        error_log("Debug handleAiGeneration: POST data = " . print_r($_POST, true));
        ...
        if (!empty($userPrompt) && !empty($userTableName)) {
            ...
            try {
                ...
                if (!function_exists('createApiRequest')) {
                    $configPath = __DIR__ . '/../../../api_config.php';
                ...
                // Use multilingual prompt template with user-specified table name
                ...
                if ($content) {
                    // Clean and validate SQL statement
                    ...
                    // Ensure SQL uses user-specified table name
                    ...
                    if (stripos($aiGeneratedSql, 'CREATE TABLE') !== false) {
                        ...
                        // Parse SQL to generate table preview
                        $aiTablePreview = $this->parseCreateTableSql($aiGeneratedSql);
                        
                        // Ensure preview uses user-specified table name
                        if ($aiTablePreview) {
    ...
    /**
     * Ensure SQL uses correct table name
     */
    private function ensureCorrectTableName(string $sql, string $correctTableName): string
    {
        // Match and replace table name in CREATE TABLE statement
        $sql = preg_replace(
    ...
    /**
     * Parse CREATE TABLE SQL statement to generate table preview
     */
    private function parseCreateTableSql(string $sql): ?array
    {
        try {
            ...
            if (preg_match('/ENGINE\s*=\s*([a-zA-Z0-9_]+)/i', $sql, $engineMatch)) {
                $engine = $engineMatch[1];
            }
            
            if (preg_match('/(?:DEFAULT\s+)?CHARSET\s*=\s*([a-zA-Z0-9_]+)/i', $sql, $charsetMatch)) {
                $charset = $charsetMatch[1];
            }
            
            // Extract column definitions
            $columns = [];
            if (preg_match('/\((.*)\)\s*(?:ENGINE|$)/is', $sql, $columnMatch)) {
                $columnText = $columnMatch[1];
                
                // Split by comma, but be careful with function calls and constraints
                $lines = preg_split('/,\s*(?=\w)/', $columnText);
                
                foreach ($lines as $line) {
                    $line = trim($line);
                    if (empty($line) || stripos($line, 'PRIMARY KEY') !== false || stripos($line, 'UNIQUE') !== false || stripos($line, 'INDEX') !== false) {
                        continue;
                    }
                    
                    // Parse column definition
                    if (preg_match('/^`?([a-zA-Z0-9_]+)`?\s+([^\s]+)(?:\s+(.*))?$/', $line, $colMatch)) {
                        $columnName = $colMatch[1];
                        $columnType = $colMatch[2];
                        $columnExtra = isset($colMatch[3]) ? $colMatch[3] : '';
                        
                        $isNull = !stripos($columnExtra, 'NOT NULL');
                        $hasDefault = stripos($columnExtra, 'DEFAULT') !== false;
                        $isAutoIncrement = stripos($columnExtra, 'AUTO_INCREMENT') !== false;
