...
class __TwigTemplate_b534b007287bc815aeb37308e482d38fbee7c7204a3634e29ce1b582ee2f3367 extends Template
{
    private $source;
    private $macros = [];
...
";
  ...
  <fieldset class=\"pma-fieldset\">
    <legend id=\"ai_analysis_header\">
      ";
            // line 636
            yield PhpMyAdmin\Html\Generator::getIcon("b_tblops", "", true);
            yield " AI Structure Analysis
    </legend>
    <div class=\"card\">
      <div class=\"card-body\">
        <p class=\"card-text text-muted\">
          Describe your table's intended purpose or requirements in natural language, and AI will analyze the current structure for potential improvements.
        </p>
        
        <div class=\"mb-3\">
          <label for=\"tableDescription\" class=\"form-label\">Table Purpose Description:</label>
          <textarea class=\"form-control\" id=\"tableDescription\" rows=\"3\" 
                    placeholder=\"Example: This table stores user account information including login credentials, profile data, and account settings for a web application with role-based access control.\"></textarea>
        </div>
        
        <div class=\"row\">
          <div class=\"col-md-6 mb-3\">
            <label for=\"expectedLoad\" class=\"form-label\">Expected Data Volume:</label>
            <select class=\"form-select\" id=\"expectedLoad\">
              <option value=\"small\">Small (< 10K records)</option>
              <option value=\"medium\" selected>Medium (10K - 1M records)</option>
              <option value=\"large\">Large (1M - 100M records)</option>
              <option value=\"enterprise\">Enterprise (> 100M records)</option>
            </select>
          </div>
          <div class=\"col-md-6 mb-3\">
            <label for=\"accessPattern\" class=\"form-label\">Primary Access Pattern:</label>
            <select class=\"form-select\" id=\"accessPattern\">
              <option value=\"read_heavy\" selected>Read Heavy</option>
              <option value=\"write_heavy\">Write Heavy</option>
              <option value=\"balanced\">Balanced Read/Write</option>
              <option value=\"analytical\">Analytical/Reporting</option>
            </select>
          </div>
        </div>
        
        <div class=\"d-flex gap-2 mb-3\">
          <button type=\"button\" class=\"btn btn-primary\" id=\"analyzeStructureBtn\">
            <i class=\"fas fa-magic\"></i> Analyze Structure
          </button>
          <button type=\"button\" class=\"btn btn-outline-secondary\" id=\"clearAnalysisBtn\">
            Clear
          </button>
        </div>
        ...
        <div id=\"analysisResults\" class=\"mt-3\" style=\"display: none;\">
          <div class=\"card border-info\">
            <div class=\"card-header bg-light\">
              <h6 class=\"mb-0\">
                <i class=\"fas fa-lightbulb text-warning\"></i> AI Analysis Results
              </h6>
            </div>
            <div class=\"card-body\">
        ...
        <div id=\"analysisLoading\" class=\"text-center mt-3\" style=\"display: none;\">
          <div class=\"spinner-border text-primary\" role=\"status\">
            <span class=\"visually-hidden\">Analyzing...</span>
          </div>
          <p class=\"mt-2 text-muted\">AI is analyzing your table structure...</p>
        </div>
      </div>
    </div>
  </fieldset>
...
\$(document).ready(function() {
    console.log('AI Structure Analysis Script loaded');
    
    \$('#analyzeStructureBtn').on('click', function() {
        const description = \$('#tableDescription').val().trim();
        const expectedLoad = \$('#expectedLoad').val();
        const accessPattern = \$('#accessPattern').val();
        
        if (!description) {
            alert('Please provide a description of your table\\'s purpose before analyzing.');
        ...
        // Send AJAX request to analyze structure
        \$.ajax({
                                    url: 'ajax_ai_structure_analysis.php',
            method: 'POST',
            data: analysisData,
            dataType: 'json',
            ...
            success: function(response) {
                ...
                \$('#analyzeStructureBtn').prop('disabled', false).html('<i class=\"fas fa-magic\"></i> Analyze Structure');
                
                if (response.success) {
                    displayAnalysisResults(response.analysis);
                    \$('#analysisResults').show();
                } else {
                    alert('Analysis failed: ' + (response.message || 'Unknown error occurred'));
                }
            },
            error: function(xhr, status, error) {
                ...
                alert(errorMsg);
                console.error('AI Analysis Error:', xhr.responseText);
            }
        });
    });
    
    \$('#clearAnalysisBtn').on('click', function() {
        \$('#tableDescription').val('');
        \$('#expectedLoad').val('medium');
        \$('#accessPattern').val('read_heavy');
        \$('#analysisResults').hide();
    });
    ...
    function displayAnalysisResults(analysis) {
        ...
        if (analysis.summary) {
            html += '<div class=\"alert alert-info\"><h6>Summary:</h6>' + analysis.summary + '</div>';
        }
        
        if (analysis.issues && analysis.issues.length > 0) {
            html += '<div class=\"alert alert-warning\"><h6><i class=\"fas fa-exclamation-triangle\"></i> Potential Issues:</h6><ul class=\"mb-0\">';
        ...
        if (analysis.recommendations && analysis.recommendations.length > 0) {
            html += '<div class=\"alert alert-success\"><h6><i class=\"fas fa-check-circle\"></i> Recommendations:</h6><ul class=\"mb-0\">';
        ...
        if (analysis.sql_suggestions && analysis.sql_suggestions.length > 0) {
            html += '<div class=\"alert alert-secondary\"><h6><i class=\"fas fa-code\"></i> Suggested SQL Modifications:</h6>';
            analysis.sql_suggestions.forEach(function(sql, index) {
                html += '<div class=\"mb-2\"><strong>Suggestion ' + (index + 1) + ':</strong></div>';
                html += '<pre class=\"bg-light p-2 rounded\"><code>' + sql + '</code></pre>';
            });
            html += '</div>';
        }
        
        if (!analysis.summary && (!analysis.issues || analysis.issues.length === 0) && 
            (!analysis.recommendations || analysis.recommendations.length === 0)) {
            html = '<div class=\"alert alert-info\">No specific issues or recommendations found. Your table structure appears to be well-designed for the described use case.</div>';
        }
